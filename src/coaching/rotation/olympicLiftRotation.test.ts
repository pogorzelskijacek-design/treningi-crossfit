...
describe('pickNextOlympicLift', () => {
  ...
  it('does not repeat the most recently used progression while others remain untried', () => {
    const history = [makeCompletedLog('tuesday', '2026-07-07', { olympicLiftProgression: OLYMPIC_LIFT_ROTATION[0] })];
  ...
  it('cycles back to the least recently used lift once every lift has been tried', () => {
    ...
    const history = OLYMPIC_LIFT_ROTATION.slice(0, -1)
      .map((progression, i) => makeCompletedLog('tuesday', `2026-06-${10 + i}`, { olympicLiftProgression: progression }))
      .reverse();
    const next = pickNextOlympicLift(history);
  ...
  it('is deterministic for identical inputs', () => {
    const history = [makeCompletedLog('tuesday', '2026-07-07', { olympicLiftProgression: 'HangClean' })];
